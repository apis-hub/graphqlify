        ...
        return new Promise(function (resolve, reject) {
            context.rootValue.client.resource('sections').read(sectionId).then(function(section){
                section.__api__.related('assets').then(function(assets){
                    assets.create('assets', { name: name, description: description, asset_data: asset_data, tag_names: tag_names }).then(function(asset){
                        resolve( {asset, sectionId, rootContext })
                    }).catch(reject);
                }).catch(reject);
            }).catch(reject);
        })
    ...
    mutateAndGetPayload: ({ id, description, asset_data,  tag_names}, context) => {
        ...
        return new Promise(function (resolve, reject) {
            context.rootValue.client.resource('assets').read(assetId).then(function (asset) {
                ...
                if (assetData) { asset.asset_data = assetData }
                if (assetTagNames) { asset.tag_names = assetTagNames }
